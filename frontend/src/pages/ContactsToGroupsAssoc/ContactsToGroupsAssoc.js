...
const ContactsToGroupsAssoc = (props) => {
  const classes = useStyles();
  const [refreshSwitch, setRefreshSwitch] = useState(false);
  const { getTokenSilently } = useAuth0();
  const [Contacts] = useFetchData(`contacts`, [refreshSwitch]);
  const [ContactGroups] = useFetchData(`contact-groups`, []);
  const [associatedGroups] = useFetchData(`contacts/assoc/contact-groups`, [
    refreshSwitch,
  ]);
  ...
  const handleSubmit = () => {
    ...
    async function writeData() {
      try {
        ...
        await axios.post(
          `${process.env.REACT_APP_ENDPOINT}/api/contacts/${activeContact.contact_ndx}/assoc/contact-groups`,
          prepareValues(),
          { headers }
        );
        if (!didCancel) {
...
export default ContactsToGroupsAssoc;
